(function ($) {
    ...
    var SelectPure = function(){
        ...
        class e {
            constructor(t,e={},s={}) {
                ...
                e.textContent&&this._setTextContent(e.textContent), this
            }
            get() {
                return this._node
            }
    ...
    $('select[multiple]').each(function()
    {
        let list = new Array();
        let id = $(this).attr("id");
        let idParent = "div-sl-multiple--" + id;
        $(this).parent().attr("id", idParent);
        ...
        var multi = new SelectPure('#' + idParent, {
            options: list,
            value: [],
            multiple: true,
            icon: "fa fa-times",
    ...
    $('.select-pure__tree button').click('click', function(e) {
        e.preventDefault();
        let isVisibility = $(this).parent().children('.tree').css('opacity');
        if (isVisibility == 0) {
            $(this).parent().children('.tree').css({
                'opacity': 1,
                'visibility': 'visible',
                'z-index': 10
            });
